...
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER"
VALUES('MEM02', 'QWER1234', '김미영', '991122-2345678', SYSDATE);
 ...
 * 
 * 1) 컬럼 레벨 설정 : 테이블 컬럼을 선언할 떄 제약조건 추가
...
-- NOT NULL + UNIQUE 제약조건의 특징이 합쳐져있음
...
CREATE TABLE USER_USED_PK(
		-- 컬럼 레벨 설정
    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO),
    ...
    -- 테이블 레벨
    -- CONSTRINT USER_NO_PK PRIMARY KEY(USER_NO)
);
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY,  -- FK 제약 조건으로 참조할 수 있는 대상
...
-- 무결성 제약조건(KH16_LSJ.GRADE_CODE_FK1)이 위배되었습니다
-- - 부모 키가 없습니다
...
CREATE TABLE USER_USED_FK2(
  ...
  REFERENES USER_GRADE2(GRADE_CODE)
  ON DELETE SET NULL
