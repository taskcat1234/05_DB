...
SELECT 
	SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1) AS 이메일아이디
FROM 
	EMPLOYEE; 
...
SELECT
	((SYSDATE - TO_DATE('2001.03.20', 'YYYY.MM.DD')) /365 + 1) AS "한국식 나이",
...
SELECT 
	TO_CHAR(CURRENT_DATE, 'YYYY/MM/DD DAY') "오늘 날짜" 
FROM DUAL;

-- 오늘 날짜를 '2025.02.20 (목) 오후 16:27:20' 변환
SELECT 
	TO_CHAR(CURRENT_DATE, 'YYYY/MM/DD (DY) AM HH:MI:SS') "오늘 날짜" 
FROM DUAL;

-- 오늘 날짜를
-- '2025년 02월 20일 (목) 오후 04시 30분 20초' 처럼 변환
SELECT 
	TO_CHAR(CURRENT_DATE, 'YYYY"년"MM"월"DD"일" (DY) AM HH"시"MI"분"SS"초"') "오늘 날짜" 
FROM DUAL;

SELECT 
	TO_CHAR(SYSDATE , 'RRRR"년"MM"월"DD"일" (DY) AM HH"시"MI"분"SS"초"') "오늘 날짜" 
FROM DUAL;
...
-- 차이 0 (50년)
SELECT 
	TO_DATE('50/02/20','YY/MM/DD') AS YY 
	,TO_DATE('50/02/20','RR/MM/DD') AS RR 
FROM DUAL;
...
SELECT '$1,500', TO_NUMBER('$1,500', '$9,999') AS 숫자 
FROM DUAL;
 ...
 *  -> NULL인경우, 아닌 경우를 놔눠서 처리
 * */

-- EMPLOTEE 테이블에서
-- 사번, 이름, 전화번호 조회
-- 단, 전화번호가 없으면 '없음'으로 조회
SELECT 
	EMP_ID ,EMP_NAME ,NVL(PHONE,'없음') AS PHONE
FROM 
	EMPLOYEE;
...
/* DECODE(컬럼명 | 계산식, 조건1, 결과 1, 
 * 조건 2, 결과 2,..... [, 아무것도 만족 X])
 * 
 * - 컬럼 값이 일치하는 조건이 있으면
 * 		해당 조건 오르쪽에 작성된 결과가 반환되는 함수
 * 
 * - swwitch, case, default과 비슷
 * */
...
-- EMPLOYEE 테이블에서
-- 사번, 이름, 급여, 구분을 조회
-- 구분을 받는 급여에 따라 초급, 중급, 고급으로 나눔
-- 급여 600만 이상 -> '고급'
-- 급여 400만 이상, 600만 미만 -> '중급'
-- 급여 400만 미만 -> '초급'

-- 단, 부서 코드가 'D6','D9'이 사원만, 부서코드 오름차순으로 조회
...
WHERE
	...
	-- SALARY >AVG(SALARY) 해당 방식은 안됨
ORDER BY
	SALARY DESC;

-- 일반적으로
-- 일반 컬럼 + 그룹함수 결과를 혼용 사용 불가 (일반적으로는 불가)
---> GROUP BY 또는 서브쿼리
