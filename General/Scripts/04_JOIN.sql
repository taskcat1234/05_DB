...
SELECT 
 DEPT_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM	
	EMPLOYEE 
JOIN
	DEPARTMENT  ON(DEPT_TITLE);
...
SELECT 
	T1.JOB_CODE , JOB_NAME, T2.JOB_CODE
FROM 
	EMPLOYEE T1
	,JOB T2
WHERE
	T1.JOB_CODE = T2.JOB_CODE ;
...
-- 사원의 급여가
-- SAL_LEVEL에 작성된 최소(MIN_SAL) ~ 최대(MAS_SAL)
-- 범위에 맞게 책정되어 있을 떄만 조회 결과(RESULT SET)에 포함하기
...
WHERE 
	DEPT_CODE = DEPT_ID -- 21행 17열
AND
	LOCATION_ID = LOCAL_CODE; -- 21행 20열
...
SELECT 
	E.EMP_NAME , J.JOB_NAME , DEPT_CODE , D.DEPT_TITLE
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
