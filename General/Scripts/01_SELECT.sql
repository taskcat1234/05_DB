...
-- EMPLOYEE 테이블에서
-- 모든 행의 이름(EMP_NAME), 급여(SALARY) 컬러 조회
 ...
 * SYSTIMESTAMP / CURRENT_TINESTAMP */
 ...
 * 
 * * DATA -> TIMESTAMP 로 바꾸면 ms단위 + 지역 정보를 추가로 얻어옴
 * */
 ...
 * - 가짜 테이블(임시 테이블)
 * - 조회할려는 데이터가 실제 테이블에 저장된 데이터가 아닌경우
 * 		사용하는 임시 테이블
 * */
...
SELECT EMP_ID AS 사번 ,
	EMP_NAME 이름,
	HIRE_DATE AS "입사한 날짜",
	FLOOR(CURRENT_DATE - HIRE_DATE) "근무 일수" -- 내림 처리
	,CEIL((CURRENT_DATE - HIRE_DATE) / 365) AS "연차" -- 올림 처리
FROM
	EMPLOYEE;
...
--------------------------------------------------------------------
/* 연결 연산자 (||)
 * - 두 컬럼값 또는 리터럴을 하나의 문자열로 연결할 떄 사용
 * */
...
WHERE 
	...
	AND
	TO_DATE('2019.12.31', 'YYYY.MM.DD') ; -- 10개 행
...
FROM 
	EMPLOYEE 
WHERE 
	EMP_NAME LIKE '%하%'; -- 정중하,하이유,하동훈,유하진
...
FROM
	EMPLOYEE 
WHERE
	EMAIL LIKE '___1_%' ESCAPE '1'; -- 12행 조회 숫자도됨
...
----------------------------------------------------------------------------
  ...
  * 
  * - 오름 차순(ASCENDING) : 점점 커지는 순서로 정렬
  * EX) 1 -> 10, A -> Z, 가 -> 하 , 과거 -> 미래
  * 
  * - 내림 차순(DESCENDING) : 점점 작아지는 순서로 정렬
  * EX) 10 -> 1, Z -> A
  */
...
-- 사번, 이름, 연봉을
-- 연봉 오름차순으로 정렬
SELECT EMP_ID AS 사번 ,EMP_NAME AS 이름,SALARY *12 연봉
FROM EMPLOYEE
ORDER BY 연봉 ASC; 
-- 별칭 이용
-- SALARY * 12 ASC
...
FROM 
	EMPLOYEE
ORDER BY
	BONUS DESC NULLS FIRST; --- ASC , DESC
	
-- 오름 차순 기본값 : NULLS LAST
-- 내림 차순 기본값 : NULLS FIRST
