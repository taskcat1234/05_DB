...
SELECT 
	DEPT_CODE, FLOOR(AVG(SALARY)) "평균 급여"  -- (4)그룹 별 평균 조회
FROM
	EMPLOYEE -- (1) 테이블 선택
WHERE 
	DEPT_CODE IN('D5', 'D6') --(2) 사용할 행 선택 (9행 선택됨)
GROUP BY
